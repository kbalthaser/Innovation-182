...
public class ResponderServiceImpl implements ResponderService {
    ...
    public Set<String> getRxNormQueryExpansionTermTypes() { //TODO this could be moved to the query implementation
        ...
        if (rxNormQueryExpansionTermTypes == null) { return rxNormQueryExpansionTermTypes; }

        rxNormQueryExpansionTermTypes = new HashSet<String>();

        Properties valueSetIds = getApplicationProperties("app.valueset.id");
        String rxNormQueryExpansionValueSetId = (String)valueSetIds
                .get("RXNORM_QUERY_EXPANSION_TERM_TYPE_CODES");
        List<ValueSetCode> valueSet = responderValueSetDao.getValueSetCodes(new BigDecimal(
                rxNormQueryExpansionValueSetId));
    ...
    public Map<String, Map<String, String>> getIndexPropertyInterpretationMap(
        ...
        for (RequestParameter requestParameter : requestParameters) {
            ...
            if (requestParameter.getParameterName() == null
                    || "".equals(requestParameter.getParameterName())
                    && requestParameter.getParameterRoot() == null
                    || "".equals(requestParameter.getParameterRoot())
                    && requestParameter.getTypeCode() == null
                    || "".equals(requestParameter.getTypeCode())) {

                System.out
                        .println("Supported request parameters must have a parameterName, parameterRoot, and typeCode. "
                                + "Invalid parameter id = "
                                + requestParameter.getRequestParameterId());
            }

            if (lastParameterRoot != null
                    && !lastParameterRoot.equals(requestParameter.getParameterRoot())) {
    ...
    public Map<String, Map<String, String>> getIndexPropertyInterpretationMap() {
        ...
        Collection<RequestParameter> requestParameters = responderRequestParameterDao
            ...
            for (String parameterValue : parameterValues) { // multiple parameters with the same name are not supported by the specification
                ...
                if (i == 0) {
                    ...
                    throw new IllegalArgumentException(
                            "Invalid request argument: there are mutiple values for "
                                    + parameterNameString
    ...
    private void containsRequiredParameters(Map<String, String> requestParameters)
            throws MissingServletRequestParameterException {
    ...
    public Properties getApplicationProperties(String propertyGroup) {
        ...
        Collection<AppProperty> appPropertyCollection = responderAppPropertyDao
                .getAppPropertyGroup(propertyGroup);
